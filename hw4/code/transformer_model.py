...
class Transformer_Seq2Seq(tf.keras.Model):
    def __init__(self, french_window_size, french_vocab_size, english_window_size, english_vocab_size):
        ...
        self.english_pos = transformer.Position_Encoding_Layer(self.english_window_size, self.embedding_size)
        self.french_pos = transformer.Position_Encoding_Layer(self.french_window_size, self.embedding_size)
    ...
    def call(self, encoder_input, decoder_input):
        ...
        french_embedding = self.french_pos(french_embedding)
        encoder = self.encoder(french_embedding)
        english_embedding = tf.nn.embedding_lookup(self.english_E, decoder_input)
        english_embedding = self.english_pos(english_embedding)
        decoder = self.decoder(english_embedding, context=encoder)
