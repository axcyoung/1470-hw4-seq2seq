...
def Attention_Matrix(K, Q, use_mask=False):
    ...
    score = tf.matmul(Q, tf.transpose(K, perm=[0,2,1])) / np.sqrt(window_size_keys)
    if use_mask==True:
...
class Atten_Head(tf.keras.layers.Layer):
    def __init__(self, input_size, output_size, use_mask):		
        ...
        self.weight_K = self.add_weight(shape=[input_size, output_size], trainable=True, name='K')
        self.weight_V = self.add_weight(shape=[input_size, output_size], trainable=True, name='V')
        self.weight_Q = self.add_weight(shape=[input_size, output_size], trainable=True, name='Q')
    ...
    def call(self, inputs_for_keys, inputs_for_values, inputs_for_queries):
        ...
        atten = Attention_Matrix(K, Q, self.use_mask)
        atten = tf.matmul(atten, V)
        return atten
